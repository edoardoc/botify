...
export class TelegramCodexBridge {
  ...
  private ensureAuthUploadAssets(): void {
    try {
      ...
      if (!fs.existsSync(serverPath)) {
        ...
        this.logger.info(`Auth receiver scaffold created at ${serverPath}`);
      }
      const injectorPath = path.join(assetsDir, 'botify_codex_inject.js');
  ...
  private renderEmbeddedIndexHtml(): string {
    const lines = [
      ...
      '<p>This embedded server exposes helpful endpoints outside Telegram:</p>',
      '<ul>',
      ...
      '</ul>',
      '<p>Refresh Codex credentials in three easy steps:</p>',
      '<ol>',
      '<li><strong>Download the injector:</strong> Use <a href="/botify_codex_inject.ps1">botify_codex_inject.ps1</a> on Windows or <a href="/botify_codex_inject.js">botify_codex_inject.js</a> with Node.js on macOS/Linux.</li>',
      '<li><strong>Run the script:</strong> It will launch <code>codex login</code> and prompt you for anything it needs.</li>',
      '<li><strong>Finish up:</strong> When the tool says “upload complete,” Botify has received your updated <code>auth.json</code>. No extra steps required.</li>',
      '</ol>',
      '<p>Advanced teams can still pull <a href="/botifyauth.js">botifyauth.js</a> for manual setups, but most users can ignore it.</p>',
      '</html>',
    ];
    return lines.join('\n');
  }
  ...
  private async handleStartAuthCommand(chatId: string, replyToMessageId?: number): Promise<void> {
    try {
      ...
      const instructions = [
        'Embedded helper server is running. Open one of the links below to access the helper tools:',
        ...urls.map((url) => `- ${url}`),
        '',
        'Highlights:',
        '• `/` – download the injector scripts plus advanced files.',
        '• `/status` – read the latest /status report in your browser.',
        '',
        'Codex refresh steps for you:',
        '1. Visit the page and download the injector that matches your setup:',
        '   • PowerShell (.ps1) for Windows (double-click friendly).',
        '   • Node.js (.js) for macOS/Linux.',
        '2. Run the injector; it will guide you through `codex login` and handle the upload automatically.',
        '3. Wait for the “upload complete” message—Botify now has your updated credentials.',
        '',
        'Need assistance? The `botifyauth.js` file on that page is available for your operations team, but you normally do not need to touch it.',
      ].join('\n');
...
function buildAuthUploadServerSource(): string {
  const lines = [
    ...
    '',
    "const HOST = process.env.BOTIFY_AUTH_HOST || '0.0.0.0';",
    'const PORT = Number(process.env.BOTIFY_AUTH_PORT || 3411);',
    "const rootDir = process.env.BOTIFY_ROOT || path.resolve(__dirname, '..');",
    ...
    "console.log('[botifyauth] Waiting for clients on ws://' + HOST + ':' + PORT);",
    '',
    "server.on('connection', (socket, request) => {",
    '  const peer = request?.socket?.remoteAddress || "unknown";',
    "  console.log('[botifyauth] client connected from ' + peer);",
    "  ensureDir(path.dirname(targetFile));",
    "  const tempFile = targetFile + '.' + Date.now() + '.tmp';",
    "  const fileStream = fs.createWriteStream(tempFile, { flags: 'w' });",
    '  let totalBytes = 0;',
    '  let finalized = false;',
    '',
    ...
    "        console.error('[botifyauth] transfer failed: ' + errorMessage);",
    '        return;',
    '      }',
    '      try {',
    "        const payload = fs.readFileSync(tempFile, 'utf8');",
    '        JSON.parse(payload);',
    "        fs.writeFileSync(targetFile, payload, 'utf8');",
    '        fs.rmSync(tempFile, { force: true });',
    "        console.log('[botifyauth] wrote ' + targetFile + ' (' + totalBytes + ' bytes)');",
    '      } catch (err) {',
    '        fs.rmSync(tempFile, { force: true });',
    "        console.error('[botifyauth] failed to finalize auth.json: ' + err.message);",
    '      }',
    '    });',
    '  }',
    '',
    ...
    "  socket.on('error', (err) => finalize(err.message));",
    '});',
    '',
    "server.on('error', (err) => { console.error('[botifyauth] server error:', err); });",
    '',
  ];
  return lines.join('\n');
}

function buildAuthInjectorSource(): string {
  const lines = [
    '#!/usr/bin/env node',
    "const fs = require('node:fs');",
    "const path = require('node:path');",
    ...
    '',
    "const HOST = process.env.BOTIFY_AUTH_HOST || process.argv[2] || '127.0.0.1';",
    'const PORT = Number(process.env.BOTIFY_AUTH_PORT || process.argv[3] || 3411);',
    "const CODEX_HOME_PATH = process.env.CODEX_HOME || path.join(process.env.USERPROFILE || process.env.HOME || process.cwd(), '.codex_mcp_home');",
    ...
    "    console.log('[botify-inject] transfer completed.');",
    '  } catch (err) {',
    "    console.error('[botify-inject] ERROR:', err.message);",
    '    process.exitCode = 1;',
    '  }',
    '})();',
    '',
    ...
    '  return new Promise((resolve, reject) => {',
    "    console.log('[botify-inject] launching `codex login`...');",
    "    const child = spawn(CODEX_BINARY, ['login'], { stdio: 'inherit' });",
    ...
    "  console.log('[botify-inject] connecting to ' + url);",
    '  return new Promise((resolve, reject) => {',
    ...
    '          return;',
    '        }',
    "        console.log('[botify-inject] payload sent, closing connection...');",
    '        socket.close();',
    '      });',
    '    });',
